  ...
  useEffect(() => {
    const fetchMyParticipations = async () => {
      try {
        ...
        alert('참여한 게시글 목록을 불러오는데 실패했습니다.');
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-10">
      <div className="mx-auto max-w-md px-6 py-8">
        <h1 className="mb-6 text-2xl font-bold text-gray-900">내가 참여한 게시글</h1>

        {posts.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <div className="mb-4 rounded-full bg-gray-100 p-6 text-4xl">🤝</div>
            <p className="text-sm font-medium text-gray-400">참여한 게시글이 없습니다.</p>
          </div>
        ) : (
